"""torchtitan 사전학습 경로 (from-scratch·full) — 수직 파이프라인의 1단계.

초소형 Qwen3 를 wikitext 로 from-scratch 사전학습한 뒤, 산출 DCP 체크포인트를 **HF 포맷**으로
export 한다(`out/<run_name>/hf`). 이 HF ckpt 가 파이프라인의 다음 단계(SFT) 입력이 된다 —
단계 간 인터페이스 = HF 체크포인트라는 설계 원칙.

스케일 knob: `model.size`(→ tfct_tiny/0.6B…) · `dataset`(wikitext-2↔103) · `hp.steps` ·
`scale.gpus`(FSDP 자동). 코드는 그대로, config 만 바꾸면 더 큰 사전학습으로 확장된다.
...
def _prepare_tokenizer_assets(cfg: RunConfig, work: Path) -> str:
    """Qwen3 토크나이저를 hf_assets 디렉토리로 저장하고 경로를 돌려준다.

    torchtitan 은 hf_assets_path 의 토크나이저로 텍스트를 토크나이즈한다. 사전학습 모델
    vocab(151936)이 Qwen3 토크나이저와 정합해야 SFT/RL 이 같은 토크나이저를 이어 쓴다(파이프라인
    정합). 사전학습은 raw text 라 chat template 은 불필요.
    """
...
def train(cfg: RunConfig) -> None:
    ...
    assets = _prepare_tokenizer_assets(cfg, work)
