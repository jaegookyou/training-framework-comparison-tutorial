...
def train(cfg: RunConfig) -> None:
    ...
    # 멀티노드(ray_bootstrap 클러스터 재사용) 시: raylet 이 조인해도 dashboard job-agent 가 늦게
    # 떠, 즉시 `ray job submit` 하면 "No available agent to submit job, please try again later" 로
    # 죽는다(2026-07-29 실측 — convert 까지 다 되고 여기서 정지). job API 가 응답할 때까지 기다린 뒤
    # submit 한다(단노드는 자체 `ray start --head` 가 준비될 때까지 블록하므로 대기 불필요).
...
def prepare(cfg: RunConfig) -> None:
    """워커 노드용 준비 — 캐논 template 을 구운 **hf-checkpoint 디렉토리**를 이 노드에 만든다.
